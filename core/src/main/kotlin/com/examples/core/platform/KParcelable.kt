...
inline fun <reified T> parcelableClassLoaderCreator(
    ...
    object : Parcelable.ClassLoaderCreator<T> {
      ...
      override fun createFromParcel(source: Parcel): T {
        val loader = T::class.java.classLoader ?: throw IllegalStateException()
...
fun Parcel.writeBigInteger(value: BigInteger?) = writeNullable(value) {
  writeByteArray(it.toByteArray())
}
...
fun <T : Parcelable> Parcel.readTypedObjectCompat(
    c: Parcelable.Creator<T>) = readNullable { c.createFromParcel(this) }

fun <T : Parcelable> Parcel.writeTypedObjectCompat(value: T?, parcelableFlags: Int) = writeNullable(
    value) { it.writeToParcel(this, parcelableFlags) }
